 ...
 *
 * Not using AJAX: You can call up truereview_entry_views_update() at any time and pass it a post ID to update the
...
add_action( 'init', 'truereview_entry_views_post_type_support' );

/* Registers the entry views extension scripts if we're on the correct page. */
add_action( 'template_redirect', 'truereview_entry_views_load' );

/* Add the entry views AJAX actions to the appropriate hooks. */
add_action( 'wp_ajax_truereview_entry_views', 'truereview_entry_views_update_ajax' );
add_action( 'wp_ajax_nopriv_truereview_entry_views', 'truereview_entry_views_update_ajax' );
 ...
 * @return void
 */
function truereview_entry_views_post_type_support() {
 ...
 * @return void
 */
function truereview_entry_views_load() {
	global $_truereview_entry_views_post_id;
	...
	if ( is_singular() && !is_preview() ) {
		...
		if ( post_type_supports( $post->post_type, 'entry-views' ) ) {
			...
			$_truereview_entry_views_post_id = get_queried_object_id();

			/* Enqueue the jQuery library. */
			wp_enqueue_script( 'jquery' );

			/* Load the entry views JavaScript in the footer. */
			add_action( 'wp_footer', 'truereview_entry_views_load_scripts' );
		}
	}
}
 ...
 * 'truereview_entry_views_meta_key' hook.
 *
 * @since 0.1.0
 * @access public
 ...
 * @return void
 */
function truereview_entry_views_update( $post_id = '' ) {
	...
	if ( !empty( $post_id ) ) {
		...
		$meta_key = apply_filters( 'truereview_entry_views_meta_key', 'Views' );
 ...
 * @return string
 */
function truereview_entry_views_get( $attr = '' ) {
...
/**
 * Callback function hooked to 'wp_ajax_truereview_entry_views' and 'wp_ajax_nopriv_truereview_entry_views'.  It checks the
 ...
 * @return void
 */
function truereview_entry_views_update_ajax() {

	/* Check the AJAX nonce to make sure this is a valid request. */
	check_ajax_referer( 'truereview_entry_views_ajax' );

	/* If the post ID is set, set it to the $post_id variable and make sure it's an integer. */
	if ( isset( $_POST['post_id'] ) )
		$post_id = absint( $_POST['post_id'] );

	/* If $post_id isn't empty, pass it to the truereview_entry_views_update() function to update the view count. */
	if ( !empty( $post_id ) )
		truereview_entry_views_update( $post_id );
}
 ...
 * @return void
 */
function truereview_entry_views_load_scripts() {
	global $_truereview_entry_views_post_id;

	/* Create a nonce for the AJAX request. */
	$nonce = wp_create_nonce( 'truereview_entry_views_ajax' );

	/* Display the JavaScript needed. */
	echo '<script type="text/javascript">/* <![CDATA[ */ jQuery(document).ready( function() { jQuery.post( "' . admin_url( 'admin-ajax.php' ) . '", { action : "truereview_entry_views", _ajax_nonce : "' . $nonce . '", post_id : ' . $_truereview_entry_views_post_id . ' } ); } ); /* ]]> */</script>' . "\n";
}
