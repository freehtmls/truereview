...
add_action( 'init', 'truereview_theme_layouts_register_meta' );

/* Add post type support for theme layouts. */
add_action( 'init', 'truereview_theme_layouts_add_post_type_support',    5 );
add_action( 'init', 'truereview_theme_layouts_remove_post_type_support', 5 );

/* Set up the custom post layouts. */
add_action( 'admin_init', 'truereview_theme_layouts_admin_setup' );

/* Add layout option in Customize. */
add_action( 'customize_register', 'truereview_theme_layouts_customize_register' );

/* Filters the theme layout mod. */
add_filter( 'theme_mod_theme_layout', 'truereview_theme_layouts_filter_layout', 5 );

/* Filters the body_class hook to add a custom class. */
add_filter( 'body_class', 'truereview_theme_layouts_body_class' );
 ...
 * @return void
 */
function truereview_theme_layouts_register_meta() {
	register_meta( 'post', truereview_theme_layouts_get_meta_key(), 'truereview_theme_layouts_sanitize_meta' );
	register_meta( 'user', truereview_theme_layouts_get_meta_key(), 'truereview_theme_layouts_sanitize_meta' );
}
 ...
 */
function truereview_theme_layouts_sanitize_meta( $meta_value, $meta_key, $meta_type ) {
 ...
 * @return void
 */
function truereview_theme_layouts_add_post_type_support() {
 ...
 * @return void
 */
function truereview_theme_layouts_remove_post_type_support() {
 ...
 */
function truereview_theme_layouts_get_layouts() {

	$layouts = get_theme_support( 'theme-layouts' );

	return isset( $layouts[0] ) ? array_keys( $layouts[0] ) : array_keys( truereview_theme_layouts_strings() );
}
 ...
 */
function truereview_theme_layouts_get_args() {
	...
	return apply_filters( 'truereview_theme_layouts_args', wp_parse_args( $args, $defaults ) );
}
 ...
 * @return string
 */
function truereview_theme_layouts_filter_layout( $theme_layout ) {
	...
	elseif ( empty( $theme_layout ) ) {
		$args = truereview_theme_layouts_get_args();
 ...
 */
function truereview_theme_layouts_get_layout() {

	/* Get the available theme layouts. */
	$layouts = truereview_theme_layouts_get_layouts();
...
function get_post_layout( $post_id ) {
	...
	/* Get the post layout. */
	$layout = get_post_meta( $post_id, truereview_theme_layouts_get_meta_key(), true );
...
function set_post_layout( $post_id, $layout ) {
	return update_post_meta( $post_id, truereview_theme_layouts_get_meta_key(), $layout );
}
...
function delete_post_layout( $post_id ) {
	return delete_post_meta( $post_id, truereview_theme_layouts_get_meta_key() );
}
...
function get_user_layout( $user_id ) {
	...
	/* Get the user layout. */
	$layout = get_user_meta( $user_id, truereview_theme_layouts_get_meta_key(), true );
...
function set_user_layout( $user_id, $layout ) {
	return update_user_meta( $user_id, truereview_theme_layouts_get_meta_key(), $layout );
}
...
function delete_user_layout( $user_id ) {
	return delete_user_meta( $user_id, truereview_theme_layouts_get_meta_key() );
}
 ...
 */
function truereview_theme_layouts_body_class( $classes ) {

	/* Adds the layout to array of body classes. */
	$classes[] = sanitize_html_class( truereview_theme_layouts_get_layout() );
 ...
 */
function truereview_theme_layouts_strings() {

	/* Set up the default layout strings. */
	$strings = array(
		/* Translators: Default theme layout option. */
		'default' => _x( 'Default', 'theme layout', 'truereview' )
	);
	...
	return apply_filters( 'truereview_theme_layouts_strings', $strings );
}
 ...
 * @return string
 */
function truereview_theme_layouts_get_string( $layout ) {

	/* Get an array of post layout strings. */
	$strings = truereview_theme_layouts_strings();
 ...
 * @return void
 */
function truereview_theme_layouts_admin_setup() {
	...
	add_action( 'load-post.php',     'truereview_theme_layouts_load_meta_boxes' );
	add_action( 'load-post-new.php', 'truereview_theme_layouts_load_meta_boxes' );
	...
	if ( post_type_supports( 'attachment', 'theme-layouts' ) ) {
		...
		add_filter( 'attachment_fields_to_edit', 'truereview_theme_layouts_attachment_fields_to_edit', 10, 2 );

		/* Saves the theme layout for attachments. */
		add_filter( 'attachment_fields_to_save', 'truereview_theme_layouts_attachment_fields_to_save', 10, 2 );
	}
}
 ...
 * @return void
 */
function truereview_theme_layouts_load_meta_boxes() {

	/* Add the layout meta box on the 'add_meta_boxes' hook. */
	add_action( 'add_meta_boxes', 'truereview_theme_layouts_add_meta_boxes', 10, 2 );

	/* Saves the post format on the post editing page. */
	add_action( 'save_post',       'truereview_theme_layouts_save_post', 10, 2 );
	add_action( 'add_attachment',  'truereview_theme_layouts_save_post'        );
	add_action( 'edit_attachment', 'truereview_theme_layouts_save_post'        );
}
 ...
 * @return void
 */
function truereview_theme_layouts_add_meta_boxes( $post_type, $post ) {
	...
	if ( ( post_type_supports( $post_type, 'theme-layouts' ) ) && ( current_user_can( 'edit_post_meta', $post->ID ) || current_user_can( 'add_post_meta', $post->ID ) || current_user_can( 'delete_post_meta', $post->ID ) ) )
		add_meta_box( 'theme-layouts-post-meta-box', esc_html__( 'Layout', 'truereview' ), 'truereview_theme_layouts_post_meta_box', $post_type, 'side', 'default' );
}
 ...
 * @return void
 */
function truereview_theme_layouts_post_meta_box( $post, $box ) {

	/* Get theme-supported theme layouts. */
	$post_layouts = truereview_theme_layouts_get_layouts();
	...
	<div class="post-layout">
		...
		<div class="post-layout-wrap">
			<ul>
				<li><input type="radio" name="post-layout" id="post-layout-default" value="default" <?php checked( $post_layout, 'default' );?> /> <label for="post-layout-default"><?php echo esc_html( truereview_theme_layouts_get_string( 'default' ) ); ?></label></li>

				<?php foreach ( $post_layouts as $layout ) { ?>
					<li><input type="radio" name="post-layout" id="post-layout-<?php echo esc_attr( $layout ); ?>" value="<?php echo esc_attr( $layout ); ?>" <?php checked( $post_layout, $layout ); ?> /> <label for="post-layout-<?php echo esc_attr( $layout ); ?>"><?php echo esc_html( truereview_theme_layouts_get_string( $layout ) ); ?></label></li>
				<?php } ?>
			</ul>
		</div>
	</div><?php
 ...
 * @return void|int
 */
function truereview_theme_layouts_save_post( $post_id, $post = '' ) {
	...
	/* Get the meta key. */
	$meta_key = truereview_theme_layouts_get_meta_key();
 ...
 * @return array  $fields
 */
function truereview_theme_layouts_attachment_fields_to_edit( $fields, $post ) {
	...
	$select = '<option id="post-layout-default" value="default" ' . selected( $post_layout, 'default', false ) . '>' . esc_html( truereview_theme_layouts_get_string( 'default' ) ) . '</option>';

	/* Loop through each theme-supported layout, adding it to the select element. */
	foreach ( $post_layouts as $layout )
		$select .= '<option id="post-layout-' . esc_attr( $layout ) . '" value="' . esc_attr( $layout ) . '" ' . selected( $post_layout, $layout, false ) . '>' . esc_html( truereview_theme_layouts_get_string( $layout ) ) . '</option>';
	...
	$fields['theme-layouts-post-layout'] = array(
		'label'         => esc_html__( 'Layout', 'truereview' ),
		'input'         => 'html',
		'html'          => $select,
		'show_in_edit'  => false,
		'show_in_modal' => true
 ...
 * @return array  $post
 */
function truereview_theme_layouts_attachment_fields_to_save( $post, $fields ) {
 ...
 */
function truereview_theme_layouts_get_meta_key() {
	return apply_filters( 'truereview_theme_layouts_meta_key', 'Layout' );
}
 ...
 */
function truereview_theme_layouts_customize_register( $wp_customize ) {
	...
	if ( true === $args['customize'] ) {
		...
		$wp_customize->add_section(
			'layout',
			array(
				'title'      => esc_html__( 'Layout', 'truereview' ),
				'priority'   => 30,
		...
		if ( 'default' == $args['default'] )
			$layout_choices['default'] = truereview_theme_layouts_get_string( 'default' );

		/* Loop through each of the layouts and add it to the choices array with proper key/value pairs. */
		foreach ( $layouts as $layout )
			$layout_choices[$layout] = truereview_theme_layouts_get_string( $layout );

		/* Add the layout control. */
		$wp_customize->add_control(
			'theme-layout-control',
			array(
				'label'    => esc_html__( 'Global Layout', 'truereview' ),
				'section'  => 'layout',
		...
		if ( $wp_customize->is_preview() && !is_admin() )
			add_action( 'wp_footer', 'truereview_theme_layouts_customize_preview_script', 21 );
	}
}
 ...
 * @return    void
 */
function truereview_theme_layouts_customize_preview_script() { ?>
